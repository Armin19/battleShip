...
public class BattleshipClient {
    ...
    public void listen() throws IOException {
        while (!(start = this.in.readLine()).equals("play")) {
            ...
            if (start.equals("getmove"))
                ...
                BattleShipFrame.getPlayers(1).setUser(in.readLine());
            }
            System.out.println("I'm still in the loop man.");
        }
        if (!start.equals("getmove")) {
            BattleShipFrame.getPlayers(0).setMove(true);
            JOptionPane.showMessageDialog(null, BattleShipFrame.getPlayers(0).getUser()
    ...
    public void results() {
        ...
        if (results.startsWith("miss")) {
            BattleShipFrame.getPlayers(1).setBboard(x, y, Color.blue);
            BattleShipFrame.getPlayers(0).setShots();
        } else if (results.startsWith("hit")) {
            BattleShipFrame.getPlayers(0).setHits();
            results = results.substring(results.indexOf(" ") + 1);
            if (results.startsWith("shipsunk")) {
                BattleShipFrame.getPlayers(1).setShipsLeft();
                results = results.substring(results.indexOf(" ") + 1);
                ...
                if (Integer.parseInt(results.substring(4, 5)) == 0) {
                    for (i = x; i <= endp; i++) {
                        BattleShipFrame.getPlayers(1).setBboard(i, y, Color.black);
                    }
                } else {
                    for (i = y; i <= endp; i++) {
                        BattleShipFrame.getPlayers(1).setBboard(x, i, Color.black);
                    }
                }
            } else {
                BattleShipFrame.getPlayers(1).setBboard(x, y, Color.orange);
            }
            BattleShipFrame.getPlayers(0).setShots();
            ...
            x = Integer.parseInt(results.substring(0, 1));
            y = Integer.parseInt(results.substring(2, 3));
            if (!BattleShipFrame.getPlayers(0).getWhatShip(x, y).equals(" ")) {
                BattleShipFrame.getPlayers(1).setHits();
                if (!BattleShipFrame.getPlayers(0).isSunk(x, y
                        , BattleShipFrame.getPlayers(0).getWhatShip(x, y)))
                    BattleShipFrame.getPlayers(0).setBboard(x, y, Color.orange);
            } else
                BattleShipFrame.getPlayers(0).setBboard(x, y, Color.blue);
            BattleShipFrame.getPlayers(1).setShots();
        ...
        try {
            if (in.readLine().equals("gameover")) {
                BattleShipFrame.setGameOver(true);
                if (BattleShipFrame.getPlayers(0).getShipsLeft() != 0)
    ...
    public void fireShot(int a, int b) {
        ...
        out.println(x);
        out.println(y);
        System.out.println("shot fired:  " + BattleShipFrame.getCletters(x + 1) + "," + BattleShipFrame.getCnumbers(y + 1));
    }
    ...
    public void sendShips() {
        out.println(BattleShipFrame.getPlayers(BattleShipFrame.getYou()).getUser());
        for (i = 0; i < 10; i++)
            for (j = 0; j < 10; j++)
                out.println(BattleShipFrame.getPlayers(BattleShipFrame.getYou()).getWhatShip(i, j));
    }
}
